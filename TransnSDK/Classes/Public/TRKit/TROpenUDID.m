...
// #define OpenUDIDLog(fmt, ...) TRLog((@"%s [Line %d] " fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
// #define OpenUDIDLog(fmt, ...) TRLog((@"[Line %d] " fmt), __LINE__, ##__VA_ARGS__);
...
+ (NSString *)value
{
    return [TROpenUDID valueWithError:nil];
}
...
+ (NSString *)valueWithError:(NSError **)error
{
    ...
    for (int n = 0; n < kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB == nil) {
            ...
            NSDictionary    *dict = [TROpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID == nil) {
        if (mostReliableOpenUDID == nil) {
            ...
            //
            openUDID = [TROpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if ((availableSlotPBid != nil) && ((myRedundancySlotPBid == nil) || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict) {
            [TROpenUDID _setDict:localDict forPasteboard:slotPB];
        }
    }
